...
export class DELTA_LOG_FAIXA_UOP_STREAM extends AbstractStream {
  /**
   *
  ...
  static async run(options) {
    const filePath = `${cwd()}/eDNE_Basico/eDNE_Delta_Basico_23011/Delimitado/DELTA_LOG_FAIXA_UOP.TXT`;
    ...
    streamLoop: for await (const line of readLine) {
      ...
      switch (operation) {
        case "INS":
          await options.infra.INSERT_INTO_LOG_FAIXA_UOP(data);
          break;
        case "UPD":
          await options.infra.INSERT_INTO_LOG_FAIXA_UOP(data);
          break;
        case "DEL":
          await options.infra.DELETE_FROM_LOG_FAIXA_UOP([data[0], data[1]]);
          break;
