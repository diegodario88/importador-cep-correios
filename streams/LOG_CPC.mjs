...
export class LOG_CPC_STREAM extends AbstractStream {
  /**
   *
  ...
  static async run(options) {
    const filePath = `${cwd()}/eDNE_Basico/eDNE_Basico_23012/Delimitado/LOG_CPC.TXT`;
    ...
    await options.infra.CREATE_TABLE_LOG_CPC();
    ...
    for await (const line of readLine) {
      ...
      await options.infra.INSERT_INTO_LOG_CPC(data.map(falsyToNull));
      bar.increment();
    }
