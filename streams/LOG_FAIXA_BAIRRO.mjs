...
export class LOG_FAIXA_BAIRRO_STREAM extends AbstractStream {
  /**
   *
  ...
  static async run(options) {
    const filePath = `${cwd()}/eDNE_Basico/eDNE_Basico_23012/Delimitado/LOG_FAIXA_BAIRRO.TXT`;
    ...
    await options.infra.CREATE_TABLE_LOG_FAIXA_BAIRRO();
    ...
    for await (const line of readLine) {
      ...
      await options.infra.INSERT_INTO_LOG_FAIXA_BAIRRO(data.map(falsyToNull));
      bar.increment();
    }
