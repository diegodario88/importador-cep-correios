...
export class LOG_FAIXA_UF_STREAM extends AbstractStream {
  /**
   *
  ...
  static async run(options) {
    const filePath = `${cwd()}/eDNE_Basico/eDNE_Basico_23012/Delimitado/LOG_FAIXA_UF.TXT`;
    ...
    await options.infra.CREATE_TABLE_LOG_FAIXA_UF();
    ...
    for await (const line of readLine) {
      ...
      await options.infra.INSERT_INTO_LOG_FAIXA_UF(data.map(falsyToNull));
      bar.increment();
    }
