...
export class LOG_GRANDE_USUARIO_STREAM extends AbstractStream {
  /**
   *
  ...
  static async run(options) {
    const filePath = `${cwd()}/eDNE_Basico/eDNE_Basico_23012/Delimitado/LOG_GRANDE_USUARIO.TXT`;
    ...
    await options.infra.CREATE_TABLE_LOG_GRANDE_USUARIO();
    ...
    for await (const line of readLine) {
      ...
      await options.infra.INSERT_INTO_LOG_GRANDE_USUARIO(data.map(falsyToNull));
      bar.increment();
    }
