...
export class LOG_VAR_LOG_STREAM extends AbstractStream {
  /**
   *
  ...
  static async run(options) {
    const filePath = `${cwd()}/eDNE_Basico/eDNE_Basico_23012/Delimitado/LOG_VAR_LOG.TXT`;
    ...
    await options.infra.CREATE_TABLE_LOG_VAR_LOG();
    ...
    for await (const line of readLine) {
      ...
      await options.infra.INSERT_INTO_LOG_VAR_LOG(data.map(falsyToNull));
      bar.increment();
    }
