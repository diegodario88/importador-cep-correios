...
export class LOG_VAR_BAI_STREAM extends AbstractStream {
  /**
   *
  ...
  static async run(options) {
    const filePath = `${cwd()}/eDNE_Basico/eDNE_Basico_23012/Delimitado/LOG_VAR_BAI.TXT`;
    ...
    await options.infra.CREATE_TABLE_LOG_VAR_BAI();
    ...
    for await (const line of readLine) {
      ...
      await options.infra.INSERT_INTO_LOG_VAR_BAI(data.map(falsyToNull));
      bar.increment();
    }
